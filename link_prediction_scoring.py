...
'''Loops over saved LDM models, calculates ROC-AUC and PR-AUC for each model'''
if __name__=='__main__':
    ...
    for j in range(9,10):
        ...
        #skal også importere rigtig ap edgelist måske
            ...
            model.load_state_dict(torch.load(fr'./Embeddings/Models/p2p_1.0_{i}', map_location = device))
            ...
            roc_auc.append(roc_auc_score(y_true=labels, y_score=pred_scores))
                # f.write(f"Roc_AUC: {roc_auc}\n")
            pr_auc.append(average_precision_score(y_true=labels, y_score=pred_scores))
                # f.write(f"PR_AUC: {pr_auc}\n")

        print(f"roc_auc for 0.{j}: ")
        print(roc_auc)
        print(f"pr_auc for 0.{j}: ")
        print(pr_auc)
